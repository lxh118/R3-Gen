...
# InternVL3.5 兼容补丁
# ---------------------------------------------------------------------------
# InternVL3.5-8B 使用 Qwen2TokenizerFast 作为底层 tokenizer，
# 但 transformers 内置的 InternVLProcessor.__init__ 需要从 tokenizer 上读取
# 7 个 InternVL 专用属性（start_image_token, end_image_token,
# start_image_token_id, end_image_token_id, context_image_token,
# context_image_token_id, video_token）。
# Qwen2TokenizerFast 本身没有这些属性，因此在这里做 class-level monkey-patch。
# 所有 token 字符串与 InternVL3_5-8B/special_tokens_map.json 保持一致。
# ---------------------------------------------------------------------------
try:
    from transformers import Qwen2TokenizerFast  # type: ignore[attr-defined]

    # ---- 文本标记 ----
    if not hasattr(Qwen2TokenizerFast, "start_image_token"):
        Qwen2TokenizerFast.start_image_token = "<img>"  # type: ignore[attr-defined]
    if not hasattr(Qwen2TokenizerFast, "end_image_token"):
        Qwen2TokenizerFast.end_image_token = "</img>"  # type: ignore[attr-defined]
    if not hasattr(Qwen2TokenizerFast, "context_image_token"):
        Qwen2TokenizerFast.context_image_token = "<IMG_CONTEXT>"  # type: ignore[attr-defined]
    if not hasattr(Qwen2TokenizerFast, "video_token"):
        Qwen2TokenizerFast.video_token = "<video>"  # type: ignore[attr-defined]

    # ---- ID 标记（property，从 vocab 动态查询，不写死 ID）----
    def _make_token_id_property(token_attr: str):
        """返回一个 property，读取时自动 convert_tokens_to_ids。"""
        def _getter(self):  # type: ignore[override]
            return self.convert_tokens_to_ids(getattr(self, token_attr))
        return property(_getter)

    for _id_attr, _tok_attr in [
        ("start_image_token_id", "start_image_token"),
        ("end_image_token_id", "end_image_token"),
        ("context_image_token_id", "context_image_token"),
    ]:
        if not hasattr(Qwen2TokenizerFast, _id_attr):
            setattr(Qwen2TokenizerFast, _id_attr, _make_token_id_property(_tok_attr))
except Exception:
    pass
