...
class Runner:
    ...
    def run(self, config: PPOConfig):
        ...
        # InternVL 固定分辨率：必须在 create_dataloader 前应用，确保 dataset 和 meta_info 都使用
        if (
            config.data.internvl_fixed_resolution is not None
            and processor is not None
            and "InternVL" in processor.__class__.__name__
        ):
            fixed = config.data.internvl_fixed_resolution ** 2
            config.data.min_pixels = fixed
            config.data.max_pixels = fixed
            print(f"[INFO] InternVL 固定分辨率: {config.data.internvl_fixed_resolution}x{config.data.internvl_fixed_resolution} (min/max_pixels={fixed})")
...
def main():
    ...
    if not ray.is_initialized():
        ...
        # 强制设置 VLLM_DISABLE_SYMMETRIC_MEMORY 和 VLLM_USE_V1（如果未设置）
