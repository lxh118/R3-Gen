...
class DataConfig:
    ...
    val_max_samples: Optional[int] = None  # 限制验证集最大样本数，None表示使用全部数据
    val_filter_overlong_prompts: Optional[bool] = None  # 验证集是否过滤超长prompt，None则与filter_overlong_prompts一致，False则保留全部验证数据（超长会截断）
    train_filter_overlong_prompts: Optional[bool] = None  # 训练集是否过滤超长prompt，None则与filter_overlong_prompts一致，False则保留全部（超长会截断）
    ...
    # InternVL 固定分辨率：设为 448 时强制 min/max_pixels=448*448，解决 GRPO 下 dynamic vision tokens 与 vit_embeds 数量不匹配的 warning
    internvl_fixed_resolution: Optional[int] = None
...
class TrainerConfig:
    ...
    save_train_generations: bool = False
    """whether to save training rollout generations to JSON files"""
    train_generations_to_save: int = -1
    """number of training generations to save per step, -1 means save all"""
