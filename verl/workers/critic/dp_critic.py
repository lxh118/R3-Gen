...
from ..actor.dp_actor import _align_internvl_vision_tokens, _stack_multi_modal_inputs
...
class DataParallelPPOCritic(BasePPOCritic):
    ...
    def _forward_micro_batch(self, micro_batch: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if "image_flags" in multi_modal_inputs and "pixel_values" in multi_modal_inputs:
            input_ids, multi_modal_inputs = _align_internvl_vision_tokens(
                input_ids, multi_modal_inputs, self.critic_module
            )
    ...
    def compute_values(self, data: DataProto) -> torch.Tensor:
        ...
        values_lst = []
        if self.rank == 0:
            if not self.config.disable_tqdm:
