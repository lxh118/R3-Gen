...
class FSDPWorker(Worker):
    ...
    @staticmethod
    def _patch_internvl_vit_checkpointing(model):
        """Monkey-patch InternVisionEncoder to use use_reentrant=False for gradient checkpointing.

        The custom modeling_intern_vit.py hardcodes torch.utils.checkpoint.checkpoint()
        with the default use_reentrant=True, which is incompatible with FSDP (causes
        'tensor data not allocated' during backward). This replaces the encoder's forward
        to pass use_reentrant=False.
        """
        import types
        ...
        def _patched_encoder_forward(self, inputs_embeds, output_hidden_states=None, return_dict=None):
    ...
    def _build_model_optimizer(
        ...
        if role != "ref":  # ref model's tokenizer is same as actor
            ...
            if getattr(self.model_config, "_name_or_path", None) in (None, ""):
                self.model_config._name_or_path = model_config.model_path
            if getattr(self.model_config, "name_or_path", None) in (None, ""):
                self.model_config.name_or_path = model_config.model_path
        ...
        # Some VLMs are not validated on the padding_free path. Prefer the standard
        # padded forward for stability until model-specific support is confirmed.
        model_type = getattr(self.model_config, "model_type", None)

        # InternVL custom model (modeling_internvl_chat) is incompatible with padding_free:
        # unpad merges batch into (1, total_nnz), breaking image-token alignment and img_context_token_id lookup.
        if model_type == "internvl_chat":
            padding_free = False
            self.print_rank0("InternVL detected: disabling padding_free for compatibility.")
        elif model_type in ("llavaonevision1_5", "llava_onevision"):
            padding_free = False
            self.print_rank0("LLaVA-OneVision detected: disabling padding_free for compatibility.")
        ...
        # InternVL3.5 custom model (modeling_internvl_chat) sets img_context_token_id only in chat();
        # forward() requires it. Set it from tokenizer so compute_log_prob works.
        _model = model.model if hasattr(model, "model") else model
        if hasattr(_model, "img_context_token_id") and _model.img_context_token_id is None:
            if self.tokenizer is not None and hasattr(self.tokenizer, "convert_tokens_to_ids"):
                _model.img_context_token_id = self.tokenizer.convert_tokens_to_ids("<IMG_CONTEXT>")
                self.print_rank0(f"Set InternVL img_context_token_id={_model.img_context_token_id}")

        # 显式转换 vision 部分为 bfloat16，确保 Flash Attention 2 兼容性
        # 问题根源：多模态模型（如 Qwen2_5_VL）的 vision_config 在配置文件中 dtype 被硬编码为 float32
        # 即使 from_pretrained 传入 torch_dtype，transformers 可能不会自动转换 vision 子模块
        # 这会导致 Flash Attention 2 警告，因为 FA2 要求所有模块使用 float16/bfloat16
        # 解决方案：显式转换 vision 部分，兼容所有 transformers 版本（>=4.40.0 支持 visual 属性）
        if torch_dtype in (torch.bfloat16, torch.float16):
            vision_converted = False
            # 兼容不同版本的 transformers 结构
            if hasattr(model, "model") and hasattr(model.model, "visual"):  # transformers >= 4.52.0 (新结构)
                if model.model.visual.dtype != torch_dtype:
                    model.model.visual = model.model.visual.to(torch_dtype)
                    vision_converted = True
            elif hasattr(model, "visual"):  # transformers < 4.52.0 (旧结构)
                if model.visual.dtype != torch_dtype:
                    model.visual = model.visual.to(torch_dtype)
                    vision_converted = True
            
            if vision_converted:
                self.print_rank0(f"Vision tower dtype converted to {torch_dtype} for Flash Attention 2 compatibility.")
        
        if model_config.enable_gradient_checkpointing:
            model.gradient_checkpointing_enable(gradient_checkpointing_kwargs={"use_reentrant": False})

        # InternVIT's custom modeling_intern_vit.py hardcodes gradient_checkpointing=True
        # with use_reentrant=True (the default), which is incompatible with FSDP and causes
        # "tensor data not allocated" during backward. Monkey-patch to use_reentrant=False.
        self._patch_internvl_vit_checkpointing(_model)
        ...
        if "multi_modal_inputs" not in self._cache:
            ...
            for index, multi_modal_data in zip(
                ...
                if index not in multi_modal_inputs_cache:
                    ...
                    if len(images) != 0:
                        is_internvl = self.processor is not None and "InternVL" in self.processor.__class__.__name__
                        ip_kwargs = {"images": images, "return_tensors": "pt"}
                        if is_internvl:
                            ip_kwargs["crop_to_patches"] = True
                        multi_modal_inputs = dict(self.processor.image_processor(**ip_kwargs))
                        if is_internvl:
                            num_patches = multi_modal_inputs.pop("num_patches", None)
                            for k in ("image_grid_thw", "video_grid_thw", "second_per_grid_ts"):
                                multi_modal_inputs.pop(k, None)
                            total_patches = sum(num_patches) if num_patches else len(images)
                            multi_modal_inputs["image_flags"] = torch.ones(total_patches, 1, dtype=torch.long)
                    elif len(videos) != 0:
                        multi_modal_inputs = dict(
                            self.processor.image_processor(images=None, videos=videos, return_tensors="pt")
                        )
                        if self.processor is not None and "InternVL" in self.processor.__class__.__name__:
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            # 处理 None 值，避免 TypeError
            if estimated_flops is None:
                estimated_flops = 0.0
            if promised_flops is None or promised_flops <= 0:
                promised_flops = 1.0  # 避免除零错误
            
            metrics["perf/mfu_actor"] = (
                estimated_flops * self.config.actor.ppo_epochs / (promised_flops * self.world_size)
            )
            freed_bytes = self.rollout_sharding_manager.freed_bytes if self.rollout_sharding_manager else 0
            metrics["perf/max_memory_allocated_gb"] = (
                torch.cuda.max_memory_allocated() - freed_bytes
            ) / (1024**3)
            metrics["perf/max_memory_reserved_gb"] = (
                torch.cuda.max_memory_reserved() - freed_bytes
            ) / (1024**3)
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        if eos_token_id is None and self.tokenizer.eos_token_id is not None:
            eos_token_id = self.tokenizer.eos_token_id
        if eos_token_id is None and hasattr(self.model_config, "eos_token_id") and self.model_config.eos_token_id is not None:
            eos_token_id = self.model_config.eos_token_id
        if eos_token_id is None and hasattr(self.model_config, "llm_config") and self.model_config.llm_config is not None:
            llm_eos = getattr(self.model_config.llm_config, "eos_token_id", None)
            if llm_eos is not None:
                eos_token_id = llm_eos
        if eos_token_id is None:
            raise ValueError(
                "eos_token_id is None. Set it in GenerationConfig, tokenizer, or model config. "
                f"generation_config.eos_token_id={getattr(self.generation_config, 'eos_token_id', None)}, "
        ...
        if pad_token_id is None and self.tokenizer.pad_token_id is not None:
            pad_token_id = self.tokenizer.pad_token_id
        if pad_token_id is None and hasattr(self.model_config, "pad_token_id") and self.model_config.pad_token_id is not None:
            pad_token_id = self.model_config.pad_token_id
        if pad_token_id is None:
            pad_token_id = eos_token_id  # fallback: many models use eos as pad
