...
def apply_ulysses_patch(model_type: str) -> None:
    # 将 qwen2_5_vl_text 映射到 qwen2_5_vl（OmniVerifier 基于 Qwen2.5-VL）
    ...
    elif model_type in ("internvl_chat",):
        # InternVL3.5 uses standard FlashAttention2 and does not need
        # any custom forward patching; we only register FA2 and keep
        # the default modeling_internvl_chat behavior.
        ALL_ATTENTION_FUNCTIONS["flash_attention_2"] = flash_attention_forward
    else:
