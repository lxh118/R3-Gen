...
# 添加：分类型的奖励客户端缓存
_omniverifier_client: Optional[RewardClient] = None
_clip_client: Optional[RewardClient] = None
...
def initialize_clients():
    ...
    global _edit_client, _reward_client, _sam3_reward_client, _omniverifier_client, _clip_client, _client_warning_printed
    ...
    if _edit_client is not None or _reward_client is not None or _sam3_reward_client is not None:
        try:
            ...
            if _check_and_reload_config(_edit_client, _reward_client, _sam3_reward_client):
                ...
                # 配置文件更新后，重置分类型的客户端缓存，让它们在下次使用时重新创建
                _omniverifier_client = None
                _clip_client = None
        except Exception as e:
            pass
...
def get_reward_client_for_type(reward_type: str):
    ...
    """
    global _reward_client, _sam3_reward_client, _omniverifier_client, _clip_client, _client_warning_printed
    
    if reward_type == "sam3":
        # 复用 RewardClient，但使用 compute_sam3_reward 接口
        return _sam3_reward_client
    elif reward_type == "omniverifier":
        # 尝试从 OMNIVERIFIER_REWARD_SERVER_ENDPOINTS 创建/获取 OmniVerifier 客户端
        ...
        if omniverifier_endpoints:
            try:
                ...
                if endpoints:
                    # 检查是否需要创建/更新客户端
                    if _omniverifier_client is None or _omniverifier_client.endpoints != endpoints:
                        ...
                        _omniverifier_client = RewardClient(
                            endpoints,
                            timeout=timeout,
                        ...
                        print(f"[INFO] OmniVerifier 客户端已更新，端点数量: {len(endpoints)}")
                    return _omniverifier_client
            except Exception as e:
                # 优化：只在第一次打印，避免重复打印
        ...
        if clip_endpoints:
            try:
                ...
                if endpoints:
                    # 检查是否需要创建/更新客户端
                    if _clip_client is None or _clip_client.endpoints != endpoints:
                        timeout = _get_env_int("API_REQUEST_TIMEOUT", 180)
                        max_retries = _get_env_int("API_MAX_RETRIES", 3)
                        health_check_timeout = _get_env_int("API_HEALTH_CHECK_TIMEOUT", 5)
                        health_check_interval = _get_env_int("API_HEALTH_CHECK_INTERVAL", 600)
                        enable_health_check = _get_env_bool("API_ENABLE_HEALTH_CHECK", True)
                        _clip_client = RewardClient(
                            endpoints,
                            timeout=timeout,
                            max_retries=max_retries,
                            health_check_timeout=health_check_timeout,
                        ...
                        print(f"[INFO] CLIP 客户端已更新，端点数量: {len(endpoints)}")
                    return _clip_client
            except Exception as e:
                # 优化：只在第一次打印，避免重复打印
...
def filter_thinking_part(response, eos_token=None):
    """
    【修改版】不再依赖 </think> 标签切割。
    直接在全文查找第一个有效的 JSON 块。
    
    返回格式兼容旧代码: (json_content, success)
    - json_content: 找到的 JSON 字符串（已去除 markdown 标记）
    - success: 是否找到了 JSON
    """
    if not response:
        return "", False
    ...
    if json_str:
        ...
        # 如果没找到 JSON，为了防止报错，返回空字符串（或者原始 response，看你偏好）
        # 建议返回空字符串，这样 accuracy_reward 就会给 0 分，而不是误判
        return "", False
...
def think_format_reward(response: str) -> float:
    ...
    规则：
    1. 必须包含 <think> 和 </think> 标签。
    2. thinking 内容非空。
    3. thinking 必须在 answer 之前。
    4. 【新增】Answer 部分必须以 '{' 或 '```' 开头，防止 garbage text 得分。
    """
    r = (response or "").strip()
    ...
    # 1. 查找标签位置
    start_idx = r.find(think_tag_start)
    end_idx = r.rfind(think_tag_end)
    ...
    # 2. 检查 Thinking 内容是否为空
    # 提取中间的内容
    thinking_content = r[start_idx + len(think_tag_start) : end_idx].strip()
    if not thinking_content:
        return 0.0
        
    # 3. 检查 Answer 部分
    # 提取 </think> 之后的内容
    answer_content = r[end_idx + len(think_tag_end):].strip()
    
    # 3.1 Answer 不能为空
    if not answer_content:
        return 0.0
        
    # 3.2 【关键修正】Answer 必须长得像有效的 JSON
    # 检查是否以 '{' 开头且有对应的 '}'，或者以 markdown 代码块 '```' 开头且有对应的 '```'
    # 这能直接过滤掉 "addCriterion..." 这种文本
    if answer_content.startswith("{"):
        # 必须有对应的闭合花括号
        if "}" not in answer_content:
            return 0.0
    elif answer_content.startswith("```"):
        # markdown 代码块必须有对应的结束标记
        if "```" not in answer_content[3:]:
            return 0.0
    else:
        # 既不是 JSON 也不是代码块
        return 0.0
        
    # 4. 额外检查：Answer 中不应该包含 <think> (防止递归/幻觉)
    if think_tag_start in answer_content:
        return 0.0
...
def json_format_reward(response: str, require_edit_prompt_for_true: bool = False) -> float:
    """
    检查 JSON 格式和必需字段奖励。
    利用 robust 的提取逻辑，无论 Thinking 在哪，只要有合法的 JSON 就进行检查。
    """
    try:
        # 1. 使用鲁棒的提取函数直接获取 JSON 字符串
        # 这会自动处理 markdown 标记，也会忽略 thinking 的位置
        json_str = filter_thinking_part(response)[0]
        
        # 如果连 JSON 块都找不到，直接 0 分
        if not json_str:
            return 0.0
        ...
        # 逻辑一致性检查：False 必须有解释
        if answer is False and not explanation:
            return 0.0
            
        # 模板文本检查 (你的原有逻辑)
        if explanation:
            explanation_lower = explanation.lower().strip()
            # ... (保留你原有的 explicit_template_patterns 检查逻辑) ...
...
def extract_edit_info(response: str) -> Optional[Dict[str, str]]:
    ...
    try:
        ...
        # ==========================================================
        # 策略4：正则暴力兜底 (新增)
        # 如果上面所有 JSON 解析都失败了（通常是因为缺逗号、引号错误），直接正则提取 key-value
        # ==========================================================
        # 匹配 "edit_prompt": "..." 格式
        pattern = r'"edit_prompt"\s*:\s*"(.*?)"'
        match = re.search(pattern, response, re.DOTALL)
        if match:
            edit_prompt_extracted = match.group(1)
            # 顺便找一下 explanation
            exp_match = re.search(r'"explanation"\s*:\s*"(.*?)"', response, re.DOTALL)
            explanation_extracted = exp_match.group(1) if exp_match else ""
            
            if is_valid_edit_prompt(edit_prompt_extracted):
                return {
                    "edit_prompt": edit_prompt_extracted.strip(),
                    "explanation": explanation_extracted.strip()
                }
...
def compute_score(
    ...
    # JSON格式检查配置
    require_edit_prompt_for_true: bool = False,  # 是否要求answer=true时也必须有edit_prompt字段
    **kwargs
) -> list[dict[str, float]]:
    """
