...
# 添加：分类型的奖励客户端缓存
_self_reward_client: Optional[RewardClient] = None
...
# 支持的奖励类型。具体 self-reward 模型由服务端 SELF_REWARD_MODEL_TYPE 决定。
...
def initialize_clients():
    ...
    global _edit_client, _reward_client, _sam3_reward_client, _self_reward_client, _clip_client, _client_warning_printed
...
def get_reward_client_for_type(reward_type: str):
    ...
    Args:
        reward_type: 奖励类型（clip、self_reward、sam3）

    Returns:
        对应的奖励客户端，如果不存在则返回 None
    """
    global _reward_client, _sam3_reward_client, _self_reward_client, _clip_client, _client_warning_printed
    ...
    if reward_type == "sam3":
        ...
        # 优先使用公开的 SELF_REWARD_SERVER_ENDPOINTS
        ...
        if self_reward_endpoints:
            try:
                ...
                if endpoints:
                    # 检查是否需要创建/更新客户端
                    if _self_reward_client is None or _self_reward_client.endpoints != endpoints:
                        ...
                        _self_reward_client = RewardClient(
                            endpoints,
                            timeout=timeout,
                        ...
                        print(f"[INFO] self-reward 客户端已更新，端点数量: {len(endpoints)}")
                    return _self_reward_client
            except Exception as e:
                # 优化：只在第一次打印，避免重复打印
        ...
        if env_reward_type in ["mixed", "clip"]:
            ...
            if not _client_warning_printed.get("self_reward_endpoints_empty", False):
                ...
                print(f"[WARNING] 建议：1) 部署 R3-Gen self-reward 服务并配置 SELF_REWARD_SERVER_ENDPOINTS")
...
def is_valid_edit_prompt(edit_prompt: str) -> bool:
    ...
    action_words = ['add', 'remove', 'replace', 'change', 'move', 'delete', 'place', 'position', 'shift', 'make', 'modify', 'update', 'swap']
...
def compute_stage2_reward_api(
    ...
    try:
        ...
        if reward_type == "sam3":
            ...
            # 并行计算 sam3 与 clip/self_reward，默认各占 50%
            if sam3_client is None:
            ...
            if base_reward_type == "mixed":
                    base_reward_type = "self_reward"  # mixed 模式下默认使用 self_reward

            # 确保 base_reward_type 不是 "mixed" 或 "sam3"（sam3 已经在 _call_sam3 中处理）
            if base_reward_type in ["mixed", "sam3"]:
                base_reward_type = "self_reward"  # 回退到 self_reward
            ...
            # 根据 reward_type 获取正确的客户端（clip 或 self_reward）
...
def compute_score(
    ...
    default_reward_type: Optional[str] = None,  # 默认奖励类型（clip或self_reward），优先级最高
    ...
    **kwargs
    ...
    Args:
        ...
        default_reward_type: 默认奖励类型（clip或self_reward）。如果设置，将覆盖数据中的reward_type和category映射。
                            优先级：default_reward_type > ground_truth中的reward_type > category映射 > "self_reward"
    ...
    for idx, reward_input in enumerate(reward_inputs):
        ...
        if enable_stage2:
            try:
                ...
                category = gt_data.get("category", "")
                # 确定奖励类型（优先级：外部参数 > ground_truth中的reward_type > category映射 > "self_reward"）
                if default_reward_type is not None:
                    # 如果设置了外部默认奖励类型，优先使用
    ...
    # 只有在 clip/self_reward 模式下，才可能需要外部的 reward_client
    ...
    if stage2_tasks and enable_stage2 and edit_client is not None:
        ...
        def process_stage2_task(task: Dict[str, Any]) -> Tuple[int, Dict[str, Any]]:
            ...
            try:
                ...
                # 无论使用什么奖励类型（clip、self_reward、sam3、mixed），都应该归类为TN
    ...
    for score in scores:
        ...
        if stage2_type == "TP":
            ...
            # 统一处理：TN（已统一命名）、clip、self_reward、sam3、mixed
    ...
    if stage2_type_counts:
        ...
        # 统一命名：将所有通过图像编辑计算的类型（clip、self_reward、sam3、mixed）合并为 TN 显示
