...
# 启动BAGEL服务（节点2，支持文生图和图片编辑）
start_edit_server() {
    echo "========================================="
    echo "启动BAGEL服务（文生图/图片编辑）"
    echo "========================================="
    
    local model_path="${1:-${EDIT_MODEL_PATH:-/path/to/BAGEL-7B-MoT}}"
    local model_type="${2:-bagel}"
    ...
    # 根据model_type选择服务器脚本
    local server_script=""
    if [[ "$model_type" == "qwen" || "$model_type" == "qwen_edit" ]]; then
        server_script="${DISTRIBUTED_SERVICES_ROOT}/servers/qwen_server.py"
    else
        server_script="${DISTRIBUTED_SERVICES_ROOT}/servers/bagel_server.py"
    fi
    ...
    # 检查服务状态并打印信息
    check_and_print_service_status "BAGEL" "BAGEL模型较大，加载可能需要2-5分钟，请耐心等待" \
...
# 启动奖励服务（支持CLIP、OmniVerifier、SAM3、mixed，支持按GPU指定类型）
start_reward_server() {
    echo "========================================="
    echo "启动奖励服务"
    echo "========================================="
    ...
    local reward_type="${REWARD_TYPE:-clip}"
    ...
    local omniverifier_model_path="${OMNIVERIFIER_MODEL_PATH:-${MODEL_PATH:-}}"
    ...
    if [[ -n "$reward_type_per_gpu" ]]; then
        ...
        for gpu_id in $(seq 0 $((num_gpus - 1))); do
            ...
            if [[ "$gpu_reward_type" == "sam3" ]]; then
                ...
                has_clip=true
            elif [[ "$gpu_reward_type" == "omniverifier" ]]; then
                has_omniverifier=true
            fi
        done
        ...
        if [[ "$has_omniverifier" == "true" ]]; then
            if [[ -z "$omniverifier_model_path" || ! -d "$omniverifier_model_path" ]]; then
                echo "[ERROR] OmniVerifier模型路径不存在或未设置: $omniverifier_model_path"
                echo "请在 config.yaml 的 service.omniverifier_model_path 中设置 OmniVerifier 模型路径"
                exit 1
            fi
        fi
    else
        # 单一模式：根据reward_type验证参数
        if [[ "$reward_type" == "omniverifier" ]]; then
            if [[ -z "$omniverifier_model_path" || ! -d "$omniverifier_model_path" ]]; then
                echo "[ERROR] OmniVerifier模型路径不存在或未设置: $omniverifier_model_path"
                echo "请在 config.yaml 的 service.omniverifier_model_path 中设置 OmniVerifier 模型路径"
                exit 1
    ...
    for gpu_id in $(seq 0 $((num_gpus - 1))); do
        ...
        case "$gpu_reward_type" in
            sam3)
                ...
                ;;
            omniverifier)
                server_script="${DISTRIBUTED_SERVICES_ROOT}/servers/omniverifier_reward_server.py"
                # server_script="${DISTRIBUTED_SERVICES_ROOT}/servers/qwen3vl_reward_server.py" 
        ...
        for instance_id in $(seq 0 $((instances_per_gpu - 1))); do
            ...
            local log_file="${LOG_DIR}/reward_server_${node_id}_gpu${gpu_id}_inst${instance_id}_${gpu_reward_type}.log"
            
            echo "启动 GPU $gpu_id 上的实例 $instance_id/$((instances_per_gpu - 1))（类型: $gpu_reward_type），端口: $port"
            
            # 启动对应类型的服务
            case "$gpu_reward_type" in
                omniverifier)
                    CUDA_VISIBLE_DEVICES=$gpu_id nohup "$python_cmd" -u "$server_script" \
                        --model_path "$omniverifier_model_path" \
                        --port "$port" \
                        --host "0.0.0.0" \
                        --device "$gpu_id" \
                        > "$log_file" 2>&1 &
...
generate_reward_server_config() {
    ...
    local omniverifier_config_file="${CONFIG_DIR}/omniverifier_reward_server_endpoints.txt"
    ...
    if [[ ${#reward_instance_info[@]} -gt 0 ]]; then
        for info in "${reward_instance_info[@]}"; do
            ...
            case "$reward_type" in
                sam3)
                    ...
                    ;;
                omniverifier)
                    target_file="$omniverifier_config_file"
                    existing_array_name="existing_omniverifier_endpoints"
                    count_var_name="added_omniverifier_count"
                    ;;
                *)
        ...
        # 向后兼容：每个GPU一个实例
        local default_reward_type="${REWARD_TYPE:-clip}"
        local target_file=""
        case "$default_reward_type" in
            sam3) target_file="$sam3_config_file" ;;
            omniverifier) target_file="$omniverifier_config_file" ;;
            *) target_file="$clip_config_file" ;;
        esac
        ...
        for gpu_id in $(seq 0 $((num_instances - 1))); do
            ...
            append_endpoint_to_file "$config_file" "$endpoint" "existing_endpoints" "added_count" >/dev/null
            append_endpoint_to_file "$target_file" "$endpoint" "existing_clip_endpoints" "added_clip_count" >/dev/null
        done
    fi
    
    # 统计各类型端点总数
    local total_clip_count=0
    local total_omniverifier_count=0
    ...
    for info in "${reward_instance_info[@]}"; do
        ...
        case "${parts[4]:-clip}" in
            ...
            omniverifier) total_omniverifier_count=$((total_omniverifier_count + 1)) ;;
    ...
    echo "配置文件路径:"
    echo "  - $config_file（统一配置文件，向后兼容）"
    echo "  - $clip_config_file（CLIP端点：当前节点 $total_clip_count 个实例）"
    echo "  - $omniverifier_config_file（OmniVerifier端点：当前节点 $total_omniverifier_count 个实例）"
    echo "  - $sam3_config_file（SAM3端点：当前节点 $total_sam3_count 个实例）"
    
    if [[ $added_count -gt 0 ]]; then
        echo ""
        echo "✅ 配置文件已更新（追加了新端点）:"
        echo "  - $config_file（追加了 $added_count 个新端点，向后兼容）"
        if [[ $added_clip_count -gt 0 ]]; then
            echo "  - $clip_config_file（追加了 $added_clip_count 个 CLIP 端点）"
        fi
        if [[ $added_omniverifier_count -gt 0 ]]; then
            echo "  - $omniverifier_config_file（追加了 $added_omniverifier_count 个 OmniVerifier 端点）"
        fi
        ...
        if [[ $total_omniverifier_count -gt 0 ]]; then
            echo "  - OmniVerifier端点: $total_omniverifier_count 个实例已配置"
        fi
...
# 停止BAGEL服务（只停止图像编辑服务）
stop_edit_server() {
    echo "停止BAGEL服务（图像编辑服务）..."
    
    # 停止BAGEL服务（更精确的匹配）
    pkill -f "distributed_services/servers/bagel_server.py" || pkill -f "bagel_server.py" || pkill -f "image_edit_server.py" || true
...
stop_reward_server() {
    ...
    # 停止奖励服务（CLIP / OmniVerifier / SAM3）
    pkill -f "distributed_services/servers/clip_reward_server.py" || pkill -f "clip_reward_server.py" || true
    pkill -f "distributed_services/servers/omniverifier_reward_server.py" || pkill -f "omniverifier_reward_server.py" || true
...
# 读取奖励服务端点（分类读取）
# 返回: 通过全局变量 REWARD_ENDPOINTS_CLIP, REWARD_ENDPOINTS_OMNIVERIFIER, REWARD_ENDPOINTS_SAM3
read_reward_server_endpoints() {
    ...
    declare -A seen_omniverifier_endpoints
    declare -A seen_sam3_endpoints
    
    local reward_endpoints_clip=""
    local reward_endpoints_omniverifier=""
    ...
    if [[ -f "$omniverifier_config_file" ]]; then
        has_classified_configs=true
        echo "[INFO] 从分类配置文件读取 OmniVerifier 服务端点: $omniverifier_config_file"
        reward_endpoints_omniverifier=$(read_endpoints_from_file "$omniverifier_config_file" "seen_omniverifier_endpoints")
    fi
            ...
            if [[ -n "$line" && "$line" =~ ^http:// ]]; then
                ...
                if [[ "$line" =~ :([0-9]+)$ ]]; then
                    ...
                    if [[ $port -ge $sam3_base_port ]]; then
                        ...
                        # CLIP/OmniVerifier 端点 - 默认作为 CLIP（向后兼容）
    ...
    # 计算并输出各类型端点数量
    local clip_count=0
    local omniverifier_count=0
    local sam3_count=0
    
    if [[ -n "$reward_endpoints_clip" ]]; then
        clip_count=$(echo "$reward_endpoints_clip" | tr ',' '\n' | grep -c '^http://' 2>/dev/null || echo "0")
    fi
    if [[ -n "$reward_endpoints_omniverifier" ]]; then
        omniverifier_count=$(echo "$reward_endpoints_omniverifier" | tr ',' '\n' | grep -c '^http://' 2>/dev/null || echo "0")
    fi
    if [[ -n "$reward_endpoints_sam3" ]]; then
    ...
    echo "[INFO] 读取到 $omniverifier_count 个 OmniVerifier 服务端点"
    echo "[INFO] 读取到 $sam3_count 个 SAM3 服务端点"
    
    # 返回三个值（通过全局变量）
    REWARD_ENDPOINTS_CLIP="$reward_endpoints_clip"
    REWARD_ENDPOINTS_OMNIVERIFIER="$reward_endpoints_omniverifier"
    REWARD_ENDPOINTS_SAM3="$reward_endpoints_sam3"
}

# 生成默认端点（向后兼容）
# 参数: $1=当前节点IP, $2=编辑服务端点变量名（通过 nameref 修改）, $3=CLIP端点变量名, $4=OmniVerifier端点变量名, $5=SAM3端点变量名
generate_default_endpoints() {
    local current_ip="$1"
    local -n edit_endpoints_ref="$2"  # nameref，直接修改传入的变量
    local -n reward_endpoints_clip_ref="$3"
    local -n reward_endpoints_omniverifier_ref="$4"
    ...
    # 生成奖励服务端点
    if [[ -z "$reward_endpoints_clip_ref" && -z "$reward_endpoints_omniverifier_ref" && -z "$reward_endpoints_sam3_ref" ]]; then
        ...
        elif [[ "$default_reward_type" == "omniverifier" ]]; then
            # 生成 OmniVerifier 端点
            for i in $(seq 0 $((num_gpus - 1))); do
                if [[ -n "$reward_endpoints_omniverifier_ref" ]]; then
                    reward_endpoints_omniverifier_ref="${reward_endpoints_omniverifier_ref},"
                fi
                reward_endpoints_omniverifier_ref="${reward_endpoints_omniverifier_ref}http://${current_ip}:$((REWARD_SERVER_BASE_PORT + i))"
            done
            echo "[INFO] 生成了 $num_gpus 个 OmniVerifier 奖励服务端点（基于当前节点: $current_ip）"
        else
            # 生成 CLIP 端点
            for i in $(seq 0 $((num_gpus - 1))); do
...
select_reward_endpoints() {
    ...
    local reward_endpoints_omniverifier="$4"
    local reward_endpoints_sam3="$5"
    
    local selected_reward_endpoints="${reward_endpoints_clip}"
    
    if [[ "$reward_type" == "omniverifier" && -n "$reward_endpoints_omniverifier" ]]; then
        selected_reward_endpoints="${reward_endpoints_omniverifier}"
        echo "[INFO] REWARD_TYPE=omniverifier，使用 OmniVerifier 服务端点" >&2
        ...
        # mixed 模式：合并 CLIP 和 OmniVerifier 端点
        selected_reward_endpoints="${reward_endpoints_clip}"
        if [[ -n "$reward_endpoints_omniverifier" ]]; then
            if [[ -n "$selected_reward_endpoints" ]]; then
                selected_reward_endpoints="${selected_reward_endpoints},${reward_endpoints_omniverifier}"
            else
                selected_reward_endpoints="${reward_endpoints_omniverifier}"
            fi
        fi
        echo "[INFO] REWARD_TYPE=mixed，合并 CLIP 和 OmniVerifier 服务端点（通过 REWARD_TYPE_PER_GPU 区分）" >&2
    else
    ...
    if [[ -n "$reward_endpoints_sam3" && -n "$selected_reward_endpoints" && "$reward_type" != "mixed" && -z "$reward_type_per_gpu" ]]; then
...
generate_service_endpoints_env() {
    ...
    local reward_endpoints_omniverifier="$5"
...
# 奖励服务端点（clip/omniverifier，根据 REWARD_TYPE 自动选择）
...
# OmniVerifier 奖励服务端点（单独配置，用于 mixed 模式或 omniverifier 模式）
export OMNIVERIFIER_REWARD_SERVER_ENDPOINTS="${reward_endpoints_omniverifier}"

# SAM3 奖励服务端点
export SAM3_REWARD_SERVER_ENDPOINTS="${reward_endpoints_sam3}"

# 奖励服务类型（clip、omniverifier、sam3 或 mixed）
...
get_config() {
    ...
    local reward_endpoints_omniverifier="$REWARD_ENDPOINTS_OMNIVERIFIER"
    local reward_endpoints_sam3="$REWARD_ENDPOINTS_SAM3"
    
    # 如果配置文件不存在或为空，生成默认端点（向后兼容）
    generate_default_endpoints "$current_ip" "edit_endpoints" "reward_endpoints_clip" "reward_endpoints_omniverifier" "reward_endpoints_sam3"
    
    # 获取奖励类型（从环境变量读取，默认ominiverifier）
    local reward_type="${REWARD_TYPE:-ominiverifier}"
    local reward_type_per_gpu="${REWARD_TYPE_PER_GPU:-}"
...
命令:
  edit_server           - 启动BAGEL服务（在节点2上运行，支持文生图和图片编辑）
                          可选参数: <model_path> <model_type>
                          示例: $0 edit_server /path/to/model bagel
  
  reward_server         - 启动奖励服务（支持CLIP和OmniVerifier/SAM3/mixed）
                          可选参数: [reward_type] [reward_type_per_gpu]
                          示例: $0 reward_server mixed "0:sam3,1:sam3,2:sam3,3:sam3,4:clip,5:clip,6:clip,7:clip"

  stop                  - 停止所有服务
  stop_edit             - 只停止BAGEL服务（图像编辑服务）
  stop_reward           - 只停止奖励服务
...
架构说明:
  ...
  节点1-N: BAGEL服务（图片编辑，每个节点8个GPU）
  节点M:   奖励服务（OmniVerifier/CLIP/SAM3/mixed，根据REWARD_TYPE和REWARD_TYPE_PER_GPU指定）
...
环境变量:
  ...
  EDIT_SERVER_NODE      - BAGEL服务节点地址（默认: 当前节点hostname）
  REWARD_SERVER_NODE    - CLIP奖励服务节点地址（默认: 当前节点hostname）

示例部署流程:
  # 在服务节点上启动BAGEL服务（单节点测试）
  ./deploy_services.sh edit_server
  
  # 在服务节点上启动奖励服务（单节点测试）
  ./deploy_services.sh reward_server mixed "0:sam3,1:clip"
...
case "$SERVICE_TYPE" in
    edit_server)
        ...
        model_type="${3:-bagel}"
        start_edit_server "$model_path" "$model_type"
