...
class Bagel(PreTrainedModel):
    ...
    def generate_image(
        ...
        if enable_taylorseer:
            from modeling.cache_utils.taylorseer import custom_cache_init
            self.language_model.model.enable_taylorseer = True
            print(f"[TaylorSeer] [ENABLED] Taylor加速已启用 (fresh_threshold={fresh_threshold}, max_order={max_order}, first_enhance={first_enhance})", flush=True)
            model_pred_cache_dic, model_pred_current = custom_cache_init(
                self, num_steps=num_timesteps,
                fresh_threshold=fresh_threshold,
                max_order=max_order,
                first_enhance=first_enhance
            )
            model_pred_text_cache_dic, model_pred_text_current = custom_cache_init(
                self, num_steps=num_timesteps,
                fresh_threshold=fresh_threshold,
                max_order=max_order,
                first_enhance=first_enhance
            )
            model_pred_img_cache_dic, model_pred_img_current = custom_cache_init(
                self, num_steps=num_timesteps,
                fresh_threshold=fresh_threshold,
                max_order=max_order,
                first_enhance=first_enhance
            ...
            print(f"[TaylorSeer] ⚠️  Taylor加速未启用 (enable_taylorseer=False)", flush=True)
    ...
    def _forward_flow(
        ...
        cfg_type: str = "parallel",
        # cache
        model_pred_cache_dic: Optional[dict] = None,
        model_pred_current: Optional[dict] = None,
        model_pred_text_cache_dic: Optional[dict] = None,
        model_pred_text_current: Optional[dict] = None,
        model_pred_img_cache_dic: Optional[dict] = None,
        model_pred_img_current: Optional[dict] = None,
    ):
        packed_text_embedding = self.language_model.model.embed_tokens(packed_text_ids)
        packed_sequence = packed_text_embedding.new_zeros(
            (sum(packed_seqlens), self.hidden_size)
