...
def auto_copy_custom_modeling_files(hf_path: str):
    ...
    custom_model_src = os.environ.get("CUSTOM_MODEL_SOURCE", "") or os.environ.get("INTERNVL_MODEL_SOURCE", "")
...
if __name__ == "__main__":
    ...
    elif "InternVLChatModel" in architectures[0] or "InternVL" in architectures[0]:
        AutoClass = AutoModel
    else:
        raise NotImplementedError(f"Unknown architecture {architectures}.")

    # Some custom vision towers call Tensor.item() during initialization, which
    # is incompatible with meta tensors. Build them on CPU before saving.
    use_cpu_init = AutoClass is AutoModel and (
        "InternVLChatModel" in architectures[0] or "InternVL" in architectures[0]
    )
