...
try:
    ...
    include_dashboard = os.environ.get("R3GEN_RAY_INCLUDE_DASHBOARD", "0").lower() in {"1", "true", "yes"}
    init_kwargs = {
    ...
    if os.environ.get("R3GEN_RAY_GPU_ACTOR", "1") == "1":
...
'''
    env = os.environ.copy()
    env["R3GEN_RAY_GPU_ACTOR"] = "1" if gpu_actor else "0"
    try:
        result = subprocess.run(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Lightweight R3-Gen training runtime diagnosis.")
...
def main() -> int:
    ...
    section("Done")
    print("Please send sections 4, 5, 6, and 7 if training still stops before Runner output.", flush=True)
    return 0
